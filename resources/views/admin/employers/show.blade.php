...
@section('title', ($employer->company_name ?? $employer->full_name ?? $employer->name) . ' — Employer')

@section('content')
<div class="mb-4">
    <a href="{{ route('admin.employers.index') }}" class="text-decoration-none text-muted">
...
@php
    ...
    $roleIcon  = $roleIcons[$employer->role]  ?? 'bi-building';
@endphp
...
<div class="row g-4">
    ...
    <div class="col-md-4">
        ...
        <div class="card-custom p-4 mb-4">
            <div class="text-center mb-3">
                @if($employer->avatar_url)
                    <img src="{{ $employer->avatar_url }}" alt="Logo"
                ...
                @endif
                <h5 class="mt-3 mb-1 fw-bold">{{ $employer->company_name ?? $employer->full_name ?? $employer->name }}</h5>
                <span class="badge bg-{{ $roleColor }}-subtle text-{{ $roleColor }} border border-{{ $roleColor }}-subtle">
                    {{ str_replace('_', ' ', ucfirst($employer->role ?? 'Employer')) }}
                </span>
            </div>
            <hr>
            <dl class="row mb-0 small">
                <dt class="col-5 text-muted">PIC Name</dt>
                <dd class="col-7">{{ $employer->full_name ?? $employer->name }}</dd>

                <dt class="col-5 text-muted">Email</dt>
                <dd class="col-7" style="word-break:break-all;">{{ $employer->email }}</dd>

                <dt class="col-5 text-muted">Phone</dt>
                <dd class="col-7">{{ $employer->phone ?? '—' }}</dd>

                @if($employer->unified_business_number)
                <dt class="col-5 text-muted">UBN</dt>
                <dd class="col-7 font-monospace">{{ $employer->unified_business_number }}</dd>
                @endif

                @if($employer->license_number)
                <dt class="col-5 text-muted">License No.</dt>
                <dd class="col-7 font-monospace">{{ $employer->license_number }}</dd>
                @endif

                @if($employer->industry)
                <dt class="col-5 text-muted">Industry</dt>
                <dd class="col-7">{{ $employer->industry }}</dd>
                @endif
                ...
                <dd class="col-7">{{ $employer->created_at?->format('M d, Y') }}</dd>

                <dt class="col-5 text-muted">Jobs Posted</dt>
                <dd class="col-7">{{ $employer->jobs()->count() }}</dd>
            </dl>
        </div>
            ...
            <div class="p-3 border-top">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <button class="btn btn-sm btn-outline-danger flex-fill"
                            onclick="openRejectModal('employer', {{ $employer->id }})">
                ...
                <button class="btn btn-sm btn-outline-warning w-100 mt-2"
                        onclick="openSuspendModal({{ $employer->id }})">
                    <i class="bi bi-slash-circle me-1"></i>Suspend Employer
                </button>
                @else
                <div class="alert alert-warning py-2 mt-2 mb-0 small">
                    <i class="bi bi-exclamation-triangle me-2"></i>
                    <strong>Account Suspended</strong>
                    <button class="btn btn-sm btn-outline-success ms-2" onclick="approveEmployer({{ $employer->id }})">Restore</button>
                </div>
                @endif
            </div>
        </div>
        ...
        {{-- Change Password --}}
        @include('admin.users.partials.change_password', ['user' => $employer])

        {{-- Jobs Summary --}}
        @php $recentJobs = $employer->jobs()->latest()->take(5)->get(); @endphp
...
<script>
    ...
    function openRejectModal(type, id) {
        _rejectType = type;
        _rejectId   = id;
        const titles = { employer: 'Reject Employer Account', emp_doc: 'Reject Document' };
    ...
    document.getElementById('rejectConfirmBtn').addEventListener('click', async () => {
        ...
        if (_rejectType === 'employer') {
            url = `/admin/employers/${_rejectId}/reject`;
        } else {
            url = `/admin/employers/documents/${_rejectId}/reject`;
        }
    ...
    async function approveEmployer(id) {
        ...
        const res = await apiCall(`/admin/employers/${id}/approve`, 'PUT');
    ...
    async function approveEmpDoc(docId) {
        ...
        const res = await apiCall(`/admin/employers/documents/${docId}/approve`, 'PUT', { note: '' });
        if (res.success) {
    ...
    async function approveAllDocs() {
        ...
        for (const btn of pendingBtns) {
            ...
            if (id) last = await apiCall(`/admin/employers/documents/${id}/approve`, 'PUT', { note: '' });
        }
    ...
    document.getElementById('suspendConfirmBtn').addEventListener('click', async () => {
        ...
        const res = await apiCall(`/admin/employers/${_rejectId}/suspend`, 'PUT', { note });
